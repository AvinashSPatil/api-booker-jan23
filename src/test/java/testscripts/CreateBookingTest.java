...
public class CreateBookingTest {
	...
	public void createBookingTestWithPOJO() {
		...
		payload.setFirstname("Avinash");
		payload.setLastname("Patil");
	...
	public void getBookingIdDeserializedTest() {
		...
		Assert.assertEquals(payload.totalprice, responseBody.totalprice);
		Assert.assertEquals(payload.depositpaid, responseBody.depositpaid);
		Assert.assertEquals(payload.bookingdates.checkin, responseBody.bookingdates.checkin);
		Assert.assertEquals(payload.bookingdates.checkout, responseBody.bookingdates.checkout);
		Assert.assertEquals(payload.additionalneeds, responseBody.additionalneeds);*/
		
	}
	
	@Test(priority=3)
	public void updateBookingIdDeserializedTest() {
		payload.setFirstname("Vipul");
		Response res = RestAssured.given()
				.headers("Content-Type","application/json")
